...
int main() {
    ...
    for (t = 1; t <= numOfMinutes; t++) 
	{
		...
		{
			thereIsLine++;
			std::cout << " wait = " << currWait/60.0 << std::endl;
			if (maxWait < currWait)
			maxWait = currWait;
		}
		else std::cout << " wait = 0;" << std::endl;
	}
    std::cout << "\nFor " << numOfClerks << " clerks, there was a line "
	 << thereIsLine/numOfMinutes*100.0 << "% of the time;" << std::endl
	 << "maximum wait time was " << maxWait/60.0 << " min." << std::endl;
    return 0;
}
